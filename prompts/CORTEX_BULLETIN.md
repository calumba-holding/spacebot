...
Make one `memory_recall` call per memory type, using the `memory_type` filter parameter. This ensures coverage across every dimension of stored knowledge. Run all eight queries:
...
8. **Todos** — `memory_recall(query: "tasks, reminders, action items, things to do", memory_type: "todo", max_results: 25)`
